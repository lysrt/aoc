...
# Having fun comparing file vs. string reading performance
def withString():
    text = '''nyot babgr babgr kqtu kqtu kzshonp ylyk psqk
...
f = float((time.time() - start_time)*1000)
print("%s ms" % f)
print("%s ms per run" % (f/N))
...
s = float((time.time() - start_time)*1000)
print("%s ms" % s)
print("%s ms per run" % (s/N))
